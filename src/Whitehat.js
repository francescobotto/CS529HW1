...
export default function Whitehat(props) {
  ...
  const mapGroupSelection = useMemo(() => {
    ...
    if (
      ...
      const getEncodedFeature = (d) => (d.count / parseInt(d.population)) * 100; // CHANGED TO USE GUN DEATHS PER CAPITA

      // Color scale based on total death per capita

      // Get the gun violence rates for all states to determine the color scale
      const stateRates = stateData.map(getEncodedFeature);
      // Get color extent for the normalized gun violence rate
      const [rateMin, rateMax] = d3.extent(stateRates);

      // Color scale based on tiotal death - NOT USED
      ...
      mapGroup
        ...
        .on("mouseover", (e, d) => {
          ...
          const entry = stateData.find((d) => d.state === sname);
          let totalDeaths = entry ? entry.count : 0; // Total deaths
          let deathRatio = entry ? (getCount(sname) * 10000).toFixed(0) : 0;
          let text = `${sname}<br>Total Gun Deaths: ${totalDeaths}<br> Rate per 10K: ${deathRatio}`;
          tTip.html(text);
        })
      ...
      // Scale based on area, so calculate a maximum area instead of a maximum radius
      const maxArea = Math.PI * Math.pow(maxRadius, 2); // Equivalent to the area of maxRadius
      const cityScale = d3
        .scaleLinear()
        .domain([0, cityMax])
        .range([0, maxArea]); // Scale up to the maximum area
      ...
      mapGroup
        .selectAll(".city")
        .data(cityData.filter((d) => d.count >= displayThreshold))
        .enter()
        .append("circle")
        .attr("class", "city")
        .attr("id", (d) => d.key)
        ...
        .attr("r", (d) => Math.sqrt(cityScale(d.count) / Math.PI)) // Convert area to radius
        .attr("opacity", 0.5)
        .on("mouseover", (e, d) => {
          // Show tooltip on hover
          const text = `City: ${d.city}<br>Gun Deaths: ${d.count}<br>`;
          tTip.html(text);
          props.ToolTip.moveTTipEvent(tTip, e);
        })
        .on("mousemove", (e) => {
      ...
      function drawLegend() {
        ...
        for (let i = 0; i <= steps; i++) {
          let rate = rateMin + i * step;
          let color = colorMap(stateScale(rate));
          let entry = {
            x: legendX,
            y: legendY,
            value: rate * 10000, // Display as per 10,000 people
            color: color,
          };
          entry.text = entry.value.toFixed(2); // Show two decimal places
